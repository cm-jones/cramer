...
public:
     ...
     * The main diagonal is the diagonal from the upper left to the lower right corner.
     *
     ...
     * In other words, the element at row i and column j is equal to the element at row j and column i, for all i and j.
     *
     ...
     * In other words, the element at row i and column j is equal to the complex conjugate of the element at row j and column i, for all i and j.
     * For real matrices, a Hermitian matrix is the same as a symmetric matrix.
     *
     ...
     * In other words, the transpose of an orthogonal matrix is equal to its inverse.
     * Orthogonal matrices have the property that they preserve the dot product of vectors and the length of vectors.
     *
     ...
     * It provides information about the matrix, such as whether the matrix is invertible and the volume of the linear transformation described by the matrix.
     * The determinant is calculated recursively using the Laplace expansion along the first row of the matrix.
     *
     ...
     *
     * LU decomposition factorizes a matrix into a lower triangular matrix (L) and an upper triangular matrix (U), such that A = LU.
     * The determinant of a triangular matrix is the product of its diagonal elements.
     * This method computes the determinant by calculating the LU decomposition and then multiplying the diagonal elements of the upper triangular matrix.
     *
     * @return The determinant of the matrix.
     */
    T det_via_lu() const;
     ...
     * For example, the element at row i and column j in the original matrix becomes the element at row j and column i in the transposed matrix.
     *
     ...
     *
     * The inverse of a square matrix A is a matrix A^(-1) such that the product of A and A^(-1) is the identity matrix.
     * Not all matrices have an inverse. A matrix is invertible (non-singular) if and only if its determinant is non-zero.
     * This method calculates the inverse using the adjugate matrix and the determinant.
     *
     ...
     * The cofactor matrix is obtained by replacing each element of the original matrix by its cofactor.
     * The cofactor of an element is the product of its minor and the sign determined by the sum of its row and column indices.
     *
     ...
     *
     * The complex conjugate of a matrix is obtained by replacing each element with its complex conjugate.
     * For real matrices, the complex conjugate is the same as the original matrix.
     *
     ...
     *
     * The matrix exponential is a matrix function that generalizes the notion of the exponential function to matrices.
     * It is defined by the power series exp(A) = I + A + A^2/2! + A^3/3! + ..., where I is the identity matrix.
     * The matrix exponential has many applications in solving systems of linear differential equations.
     *
     ...
     *
     * LU decomposition factorizes a matrix into a lower triangular matrix (L) and an upper triangular matrix (U), such that A = LU.
     * This method uses Gaussian elimination with partial pivoting to compute the LU decomposition.
     *
     ...
     *
     * QR decomposition factorizes a matrix into an orthogonal matrix (Q) and an upper triangular matrix (R), such that A = QR.
     * This method uses the Gram-Schmidt process to compute the QR decomposition.
     *
     ...
     *
     * Singular Value Decomposition (SVD) factorizes a matrix into the product of three matrices: A = U * Sigma * V^T,
     * where U and V are orthogonal matrices, and Sigma is a diagonal matrix containing the singular values.
     * SVD has many applications in signal processing, data compression, and dimensionality reduction.
     *
     * @return A tuple of matrices (U, Sigma, V) where U and V are orthogonal matrices and Sigma is a diagonal matrix.
     */
     ...
     *
     * Eigenvalues are scalar values associated with a matrix that describe the scaling factor of the eigenvectors when the matrix is multiplied by them.
     * For a square matrix A, if there exists a vector v and a scalar lambda such that Av = lambda * v, then lambda is an eigenvalue and v is the corresponding eigenvector.
     * This method computes the eigenvalues using the power iteration method.
     *
     ...
     * It represents the dimension of the vector space spanned by the rows or columns of the matrix.The rank is computed by performing singular value decomposition (SVD) and counting the number of non-zero singular values.
     *
